module UI.Utils exposing (cssColorValueToColor, colorToHexCssString, colorToInt)
...
cssColorValuetoRgb colorValue =
    let
        ...
        rawHex = String.dropLeft 1 colorValue

        naiveFromString = Result.withDefault 0 << Hex.fromString
...
cssColorValueToColor colorValue =
    let
        (r, g, b) = cssColorValuetoRgb colorValue
    in
    Color.rgb255 r g b
...
colorToHexCssString color =
    let
        ...
        intoIntValue val = round (val * 255)
        ...
        padComponent component =
            let
                len = String.length component

                missingZeroes = String.repeat (2 - len) "0"
            in
...
hexCssStringToInt hexString =
    let
        rawHex = String.dropLeft 1 hexString
    in
