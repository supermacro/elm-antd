...
import Url.Parser as P exposing (parse, oneOf, (<?>), Parser)
import Url.Parser.Query as Query

import Ant.Button as Btn exposing (button, ButtonType(..))
import Ant.Typography as Heading exposing (title)


type Model = Model Component
...
type RawComponent = RawComponent (Maybe String)
    ...
    case maybeStr of
        Just str ->
            case str of
                ...
                "PrimaryButton" -> Button { type_ = Primary, disabled = True }
                
                _ -> Button { type_ = Default, disabled = False }

        Nothing -> Button { type_ = Default, disabled = False }
...
getComponentFromUrl url =
    let
        maybeRawComponent = parse componentParser url
        rawComponent = Maybe.withDefault (RawComponent <| Just "SimpleButton") maybeRawComponent
    in
